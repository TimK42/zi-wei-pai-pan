...
- 星曜排版：主星 + 輔星左欄垂直排列，雜星右欄垂直排列（靠右對齊）
- 支援深色主題（自動跟隨系統設定）
...
- **設定持久化**：所有設定（性別、出生、目標時間、運限開關、收疊狀態）自動存入瀏覽器 localStorage，切換頁面或重新開啟後自動恢復
- 目標時間區段可收疊，收疊時時間輸入與開關隱藏，但計算持續生效
- 變更後**自動**重新計算所有運限
...
| `palaces[].majorStars` | 主星 | bold，左欄 |
| `palaces[].minorStars` | 輔星 | normal，左欄（主星下方） |
| `palaces[].adjectiveStars` | 雜曜 | lighter，右欄 |
...
- 各層獨立色系（大限橙、小限綠、流年藍、流月青、流日赭、流時紅）
